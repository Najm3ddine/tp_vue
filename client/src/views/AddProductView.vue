              ...
              <div class="needs-validation" novalidate>
                <div class="row g-2">
                  ...
                  <div v-else>
                    ...
                    <ProductView
                      v-if="!!productAdded"
                      :initial-product="productAdded" />
                    <button
                      v-if="!!productAdded"
                      class="w-100 mb-2 btn btn-danger btn-lg mt-3" type="button" @click="deleteProduct">Delete product</button>
                    <button class="w-100 mb-2 btn btn-success btn-lg mt-3" type="button" @click="newProduct">New product </button>
                  </div>
                  <hr class="my-4">
                </div>
              </div>
...
export default {
  ...
  props: ['addInv', 'inventory', 'removeInv'],
  data () {
    return {
      submitted: false,
      message: null,
  ...
  methods: {
    ...
    deleteProduct () {
      ProductDataService.delete(this.product.id)
        ...
        .catch(error => {
          console.log(error)
          this.message = error?.response?.data?.message || 'Error deleting product'
        })
    },
    newProduct () {
      this.product = {}
